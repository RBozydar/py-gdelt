...
async def gdelt_events(
    ...
    most_negative_heap: list[tuple[float, dict[str, Any]]] = []
    most_positive_heap: list[tuple[float, dict[str, Any]]] = []
    ...
    async for event in client.events.stream(event_filter):
        ...
        if len(most_negative_heap) < 5:
            heapq.heappush(most_negative_heap, (-event.goldstein_scale, event_dict))
        elif event.goldstein_scale < -most_negative_heap[0][0]:
            heapq.heapreplace(most_negative_heap, (-event.goldstein_scale, event_dict))

        # Keep 5 most positive (use min heap)
        if len(most_positive_heap) < 5:
            heapq.heappush(most_positive_heap, (event.goldstein_scale, event_dict))
        elif event.goldstein_scale > most_positive_heap[0][0]:
            heapq.heapreplace(most_positive_heap, (event.goldstein_scale, event_dict))
    ...
    if total_events > 0:
        ...
        goldstein_std = math.sqrt(max(0.0, variance))  # Avoid negative due to floating point errors
    else:
        goldstein_std = 0.0
    ...
    sample_events = []
    sample_events.extend([event_dict for _, event_dict in most_negative_heap])
    sample_events.extend([event_dict for _, event_dict in most_positive_heap])
